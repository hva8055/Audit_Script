...
"color: rgb(255, 255, 255);")
    ...
    def run(self):
        if(self.check_and_set_policy_for_user()):
            pass
        else:
            return
        try:
        ...
        # Map selections to script paths
            if os_choice == "Windows 11":
                if "L1" in level:
                    ...
                    script_path = r"D:\harsha_vardhan\projects\Audit_script\Windows\Audit_script_l1.ps1"
                elif "L2" in level:
                    script_path = r"D:\harsha_vardhan\projects\Audit_script\Windows\Audit_scriptl2.ps1"
                elif "L3" in level:
                    script_path = r"D:\harsha_vardhan\projects\Audit_script\Windows\Audit_scriptl3.ps1"
            elif os_choice == "Ubuntu":
                if "L1" in level:
                    script_path = r"" # Address for script
                elif "L2" in level:
                    script_path = r"" # Address for script
                elif "L3" in level:
                    script_path = r"" # Address for script
    ...
    def check_and_set_policy_for_user(self):
        # Check the current policy
        check_command = ["powershell", "Get-ExecutionPolicy"]
        policy_result = subprocess.run(check_command, capture_output=True, text=True, check=True)
        current_policy = policy_result.stdout.strip()

        if current_policy == "Restricted":
        # Prompt the user for permission to change
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Security Policy Warning")
            msg_box.setText(
                "The audit script cannot run with the current PowerShell security policy.\n\n"
                "Would you like to automatically set the policy to 'RemoteSigned'?"
            )
            ...
            # Command to set the policy
                set_command = ["powershell", "Set-ExecutionPolicy", "RemoteSigned", "-Force"]
                try:
                    subprocess.run(set_command, check=True)
                    QMessageBox.information(None, "Success", "Execution Policy has been updated.")
                    return True
                except subprocess.CalledProcessError:
                    QMessageBox.critical(None, "Error", "Failed to update Execution Policy. Please try running the application as an administrator.")
                    return False
            else:
                QMessageBox.warning(None, "Warning", "Audit canceled. The script cannot run without the correct policy.")
                return False
